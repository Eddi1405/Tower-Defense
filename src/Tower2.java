...
public class Tower2{
    ...
    public Tower2(UI_gamepanel gp,Tower tw) {
        this.gp = gp;
        this.tw = tw;
        pointTower = new Point[2];
        setpoint();
        Tower2.ClickListener clickListener = new Tower2.ClickListener();
        Tower2.DragListener dragListener = new Tower2.DragListener();
    ...
    public void drawTower(Graphics2D g2){
        g2.drawImage(tw.towerPic[1].image,(int) pointTower[0].getX() , (int) pointTower[0].getY(), (int) round(gp.w_TileSize * 1.0), (int) round(gp.h_TileSize * 1.0), null);
    }

    public void setpoint(){
        for(int i = 0;i < pointTower.length;i++){
            pointTower[i] = new Point(gp.screen + round(tw.breite * 13)+(int) round(gp.w_TileSize * 1.2), (int) round(gp.h_TileSize * 1.2) + 50);
        }
    }
